...
class JoinLayer(Layer):
    ...
    #     print (self.compute_output_shape(input_shape))
    #     return (input_shape[0], self.compute_output_shape(input_shape))
...
def fractal_conv(filter, nb_row, nb_col, dropout=None):
    def f(prev):
        ...
        conv=Convolution2D(filter, (nb_row, nb_col), padding="same", kernel_initializer="he_normal")(conv)
        if dropout:
